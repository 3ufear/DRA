...
public class MessageParser extends ElementParser implements IMessageParser {
 ...
 // private static final Logger logger = LoggerFactory.getLogger(MessageParser.class);
  ...
  void copyBasicAvps(IMessage newMessage, IMessage prnMessage, boolean invertPoints) {
    ...
    // Copy Application id's information
    {
      avp = prnMessage.getAvps().getAvp(Avp.VENDOR_SPECIFIC_APPLICATION_ID);
      if (avp != null) {
        newMessage.getAvps().addAvp(new AvpImpl(avp));
      }
      avp = prnMessage.getAvps().getAvp(Avp.ACCT_APPLICATION_ID);
      if (avp != null) {
        newMessage.getAvps().addAvp(new AvpImpl(avp));
      }
      avp = prnMessage.getAvps().getAvp(Avp.AUTH_APPLICATION_ID);
      if (avp != null) {
        newMessage.getAvps().addAvp(new AvpImpl(avp));
      }
    }
